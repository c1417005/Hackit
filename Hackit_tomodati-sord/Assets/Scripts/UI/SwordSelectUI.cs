...
public class SwordSelectUI : MonoBehaviour
{
    ...
    void BuildPlayerPanel(int player, Vector2 anchorMin, Vector2 anchorMax)
    {
        ...
        string[] labels = { "ATK", "DEF", "SPD", "HEIGHT" };
        Color[] colors =
        {
    ...
    void ShowSword(int player, SwordData sword)
    {
        ...
        float heightCm = TposeSwordTemplateSettings.ResolveHeightCm(sword);
        float[] values = { sword.stats.attack, sword.stats.defense, sword.stats.speed, heightCm };
        float[] maxima = { 70f, 70f, 70f, TposeSwordTemplateSettings.MaximumSupportedHeightCm };
        for (int stat = 0; stat < 4; stat++)
        {
            float ratio = Mathf.Clamp01(values[stat] / maxima[stat]);
            _barFills[player, stat].rectTransform.anchorMax = new Vector2(ratio, 1f);
            _barValues[player, stat].text = stat == 3
                ? $"{values[stat]:0}cm"
                : Mathf.RoundToInt(values[stat]).ToString();
        }
    }
